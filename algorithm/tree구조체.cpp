...
struct node { //êµ¬ì¡°ì²´ë¡œ êµ¬í˜„ 
	string position;
	std::vector<node*>children;  //?ž?‹ë¦¬ìŠ¤?Š¸ 
	node(const string& pos) :position(pos){} 
};
struct tree {
	node* root = nullptr;
	...
	static node* find(node* root, const string& pos) { //?ž¬ê·?? ?œ¼ë¡? êµ¬í˜„ 
		if (root == nullptr)   //?ž¬ê·? ì¢…ë£Œì¡°ê±´  
			return nullptr;
